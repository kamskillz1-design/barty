    ...
    <Link to={`/listings/${listing.id}`} className="group block overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-slate-200/60">
      <div className="relative aspect-[4/3] bg-slate-100">
        {listing.image_urls && listing.image_urls.length > 0 ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-slate-300">
            {listing.have_exchange_type === 'services' ? <Wrench className="h-10 w-10" /> : <Package className="h-10 w-10" />}
          </div>
        )}
        <span className={`absolute top-2.5 start-2.5 rounded-full px-2.5 py-1 text-[11px] font-bold text-white ${(listing.type || 'have') === 'want' ? 'bg-emerald-500' : 'bg-indigo-600'}`}>
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between text-xs text-slate-400">
          <span className="rounded-md bg-slate-100 px-2 py-0.5 font-medium text-slate-600">{categoryLabel(t, listing.have_category)}</span>
          {loc && (
