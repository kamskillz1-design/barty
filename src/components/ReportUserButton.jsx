...
export default function ReportUserButton({ userId, className = '' }) {
  const { t } = useI18n();
  const { user } = useAuth();
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await base44.functions.invoke('reportUser', { reported_user_id: userId, reason, note });
      const d = res?.data || res || {};
      if (d.alreadyFlagged) setAlready(true);
      setDone(true);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <>
      <button
        ...
        title={m.reportUser || 'Report user'}
      ...
      >
        <Flag className="h-3.5 w-3.5" /> {m.reportUser || 'Report user'}
      </button>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {m.reportUserTitle || 'Report this user'}
            </DialogTitle>
          ...
          {done ? (
            <div className="py-6 text-center">
              ...
              <p className="text-sm text-slate-700">
                {already ? (m.alreadyReported || '') : (m.reported || '')}
              </p>
            </div>
          ) : (
              ...
              <RadioGroup value={reason} onValueChange={setReason} className="gap-2">
                {REASONS.map((r) => (
                  <div key={r} className="flex items-center gap-2.5 rounded-xl border border-slate-200 px-3 py-2.5 hover:bg-slate-50 cursor-pointer">
                    <RadioGroupItem value={r} id={`uflag-${r}`} />
                    <Label htmlFor={`uflag-${r}`} className="cursor-pointer text-sm font-medium text-slate-700">{labels[r] || r}</Label>
                  </div>
                ))}
              </RadioGroup>
              <Textarea
