...
export default function Profile() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setSaveErr(error.message || t.profile.saveError);
    } finally {
      setSaving(false);
    }
  };
  ...
  const inputCls =
    "w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-sky-400 focus:bg-white";
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="rounded-3xl border border-slate-200 bg-white p-6">
        ...
        <div className="mt-5 flex items-center gap-3">
          ...
          <button
            onClick={() => logout(true)}
            className="ms-auto rounded-xl px-4 py-2.5 text-sm font-medium text-slate-500 hover:bg-slate-100"
          >
            {t.profile.logout}
          </button>
        </div>
              ...
              >
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 text-lg">
                  {listing.have_exchange_type === "services"
                    ? "🛠️"
                    : listing.have_exchange_type === "digital"
                    ? "💻"
                    : "📦"}
                </div>
      ...
      <div>
        <h2 className="text-lg font-bold text-slate-900">
          {t.community?.reviews?.title || t.profile.reviews}
        </h2>
