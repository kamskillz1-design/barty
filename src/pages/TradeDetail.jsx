  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 flex flex-col h-[420px]">
        <h3 className="mb-3 font-bold text-slate-900">{t.trade.chat}</h3>
        <div className="flex-1 overflow-y-auto space-y-2 pe-1">
          {messages.length === 0 && <p className="text-center text-sm text-slate-400 mt-8">{t.trade.empty}</p>}
          {messages.map((m) => {
            const mine = m.sender_id === user.id;
            return (
              <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] rounded-2xl px-3.5 py-2 text-sm ${mine ? 'bg-sky-500 text-white' : 'bg-slate-100 text-slate-800'}`}>
                  {m.text}
                </div>
              </div>
            );
